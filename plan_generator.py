# plan_generator.py - VERSION 12.1 (Syntax Fix)
import json
import math
import os
...
# (All constants and functions are unchanged from the previous version)
...
ROUTES = [{"id":"p40-p39","name":"Pier 40 to Pier 39"},{"id":"p40-clipper","name":"Pier 40 to Clipper Cove"},{"id":"p40-tiburon","name":"Pier 40 to Tiburon"},{"id":"p40-cavallo","name":"Pier 40 to Cavallo Point"},{"id":"p40-sfyh","name":"Pier 40 to San Francisco Yacht Harbor"},{"id":"p40-aqpark","name":"Pier 40 to San Francisco Aquatic Park"},{"id":"p40-torpedo","name":"Pier 40 to Torpedo Warf"},{"id":"p40-cranecove","name":"Pier 40 to Crane Cove Park"},{"id":"p40-brisbane","name":"Pier 40 to Brisbane Marina"},{"id":"p40-jacklondon","name":"Pier 40 to Jack London Square Marina"},{"id":"p40-brooklyn","name":"Pier 40 to Brooklyn Basin"},{"id":"p40-berkeley","name":"Pier 40 to Berkeley Marina"},{"id":"p40-schoonmaker","name":"Pier 40 to Schoonmaker Beach"}]
for r in ROUTES: r['legs'] = [(P40, globals()[r['id'].split('-')[1].upper().replace('SQ', 'SQUARE')])]
...
def get_noaa_predictions(station, product):
    today_str = datetime.now(TZ).strftime('%Y%m%d'); params = {"station": station, "product": product, "datum": "MLLW", "units": "english", "time_zone": "lst_ldt", "format": "json", "application": "SpinoutFairRide", "begin_date": today_str, "range": "168"}
...
def main():
    ...
    payload = {"generated_at": datetime.now(TZ).isoformat(), "rider_preset": "Casual", "version": "12.1.0-syntax-fix", "days": []}
    for d, day_forecast in enumerate(daily_forecasts):
        ...
        for r in ROUTES:
            ...
            try:
                if all_currents_today:
                    ...
                    if duration > 1.0:
                        ...
                        if currents_in_window: min_c=min(float(p['s']) for p in currents_in_window); max_c=max(float(p['s']) for p in currents_in_window); current_summary = f"{direction} {abs(min_c):.1f}-{abs(max_c):.1f} kts"
            ...
            if "Pier 40 to Pier 39" in r["name"]: rec.update({"difficulty": "No-Go", "duration_hours": 0.0, "distance_miles": round(dist, 1), "no_go_reason": "Route too short.", "current_summary":"N/A", "current_effect":"N/A"})
