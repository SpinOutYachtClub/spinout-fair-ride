# plan_generator.py - VERSION 11.0 (FINAL SYNC - FULL CODE)
import json
import math
import os
...
ROUTES = [
    # Using full, proper names for the V2 frontend
    ...
    {"id":"p40-sfyh", "name":"Pier 40 to San Francisco Yacht Harbor", "legs":[(P40, SFYH), (SFYH, P40)]},
    {"id":"p40-aqpark", "name":"Pier 40 to San Francisco Aquatic Park", "legs":[(P40, AQ_PARK), (AQ_PARK, P40)]},
    {"id":"p40-torpedo", "name":"Pier 40 to Torpedo Warf", "legs":[(P40, TORPEDO), (TORPEDO, P40)]},
    {"id":"p40-cranecove", "name":"Pier 40 to Crane Cove Park", "legs":[(P40, CRANE_COVE), (CRANE_COVE, P40)]},
    {"id":"p40-brisbane", "name":"Pier 40 to Brisbane Marina", "legs":[(P40, BRISBANE), (BRISBANE, P40)]},
    {"id":"p40-jacklondon", "name":"Pier 40 to Jack London Square Marina", "legs":[(P40, JACK_LONDON), (JACK_LONDON, P40)]},
    ...
    {"id":"p40-schoonmaker", "name":"Pier 40 to Schoonmaker Beach", "legs":[(P40, SCHOONMAKER), (SCHOONMAKER, P40)]},
]
...
def get_noaa_predictions(station, product):
    ...
    params = {"date": "today", "station": station, "product": product, "datum": "MLLW", "units": "english", "time_zone": "lst_ldt", "format": "json", "application": "SpinoutFairRide", "begin_date": today_str, "range": "192"}
    ...
    try:
        ...
        print(f"INFO: Could not fetch NOAA {product} data for {station}. Error: {e}"); return None
...
def main():
    ...
    payload = {"generated_at": datetime.now(TZ).isoformat(), "rider_preset": "Casual", "version": "11.0.0-final", "days": []}
    ...
    for d, day_forecast in enumerate(daily_forecasts):
        ...
        for r in ROUTES:
                ...
                if all_currents_today:
                    ...
                    if duration > 1.0:
                        ...
                        if currents_in_window:
                            min_c=min(float(p['s']) for p in currents); max_c=max(float(p['s']) for p in currents); current_summary = f"{direction} {abs(min_c):.1f}-{abs(max_c):.1f} kts"
            ...
            if r["id"] == "p40-p39": rec.update({"difficulty": "No-Go", "duration_hours": 0.0, "distance_miles": round(dist, 1), "no_go_reason": "Route too short.", "current_summary":"N/A", "current_effect":"N/A"})
    ...
    print("Successfully wrote new plan with all features.")
