# plan_generator.py - VERSION 8.1 (STABLE REVERT)
import json
import math
import os
...
def main():
    ...
    payload = {"generated_at": datetime.now(TZ).isoformat(), "rider_preset": "Casual", "version": "8.1-stable-revert", "days": [], "disclaimer": "Advisory only..."}
        ...
        for r in ROUTES:
            ...
            try:
                if tide_data and 'predictions' in tide_data:
                    tide_events = [p for p in tide_data['predictions'] if p and 'type' in p and start_time <= datetime.strptime(p['t'], '%Y-%m-%d %H:%M').astimezone(TZ) < end_time]
    ...
    print("Successfully wrote plan to stable revert state.")
