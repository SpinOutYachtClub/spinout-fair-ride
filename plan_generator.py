...
def get_noaa_predictions(station, product):
    ...
    params = {"begin_date": today_str, "range": "168", "station": station, "product": product, "datum": "MLLW", "units": "english", "time_zone": "lst_ldt", "format": "json", "application": "SpinoutFairRide"}
    if product == "predictions":
        params["interval"] = "hilo"
    url = f"https://api.tidesandcurrents.noaa.gov/api/prod/datagetter"
    try:
        r = requests.get(url, params=params); r.raise_for_status(); return r.json()
    except requests.exceptions.RequestException as e: print(f"INFO: Could not fetch NOAA {product}. Error: {e}"); return None
...
def main():
    ...
    payload = {"generated_at": datetime.now(TZ).isoformat(), "rider_preset": "Casual", "version": "8.0-final-parser", "days": [], "disclaimer": "Advisory only..."}
        ...
        for r in ROUTES:
            ...
            try:
                if tide_data and 'predictions' in tide_data:
                    tide_events = [p for p in tide_data['predictions'] if 'type' in p and start_time <= datetime.strptime(p['t'], '%Y-%m-%d %H:%M').astimezone(TZ) < end_time]
    ...
    print("Successfully wrote new plan with final parser fix.")
