# plan_generator.py - VERSION 7.0 (ECHO GPT 5 FIX)
import json
import math
import os
...
def get_noaa_predictions(station, product, date_str):
    ...
    # THIS IS THE CRITICAL FIX FROM ECHO
...
def main():
    ...
    payload = {"generated_at": datetime.now(TZ).isoformat(), "rider_preset": "Casual", "version": "7.0.0-echo-fix", "days": [], "disclaimer": "Advisory only..."}
        ...
        for r in ROUTES:
            ...
            try:
                if tide_data and 'predictions' in tide_data:
                    tide_events = [p for p in tide_data['predictions'] if start_time <= datetime.strptime(p['t'], '%Y-%m-%d %H:%M').astimezone(TZ) < end_time]
            ...
            if r["id"] == "p40-p39" and duration < 2.0: rec.update({"difficulty": "No-Go"}) # Simple duration rule for now
    ...
    print("Successfully wrote new plan with Echo GPT 5 fix.")
