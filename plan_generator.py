# plan_generator.py - VERSION 12.0 (ECHO + GIA FINAL)
import json
import math
import os
...
for r in ROUTES: r['legs'] = [(P40, globals()[r['id'].split('-')[1].upper()])]
...
def route_distance_miles(route): return sum(haversine_miles(a, b) for a, b in route["legs"]) * 2 # Account for round trip
...
def main():
    ...
    payload = {"generated_at": datetime.now(TZ).isoformat(), "rider_preset": "Casual", "version": "12.0.0", "days": []}
        ...
        for r in ROUTES:
            ...
            try:
                if all_currents_today:
                    ...
                    if duration > 1.0 and currents_in_window: # ECHO's BUG FIX is HERE
                    ...
                    if route_bearing is not None:
                        ...
                        if angle_diff < 45: current_effect = "Fair"; elif angle_diff > 135: current_effect = "Foul"; else: current_effect = "Cross"
            ...
            if r["name"] == "Pier 40 to Pier 39": rec.update({"difficulty": "No-Go", "duration_hours": 0.0, "distance_miles": 0.0, "current_summary":"N/A", "current_effect":"N/A", "no_go_reason": "Route too short."})
            day_obj["recommendations"].append(rec)
        payload["days"].append(day_obj)
    
    # ECHO's Flexible Publish Directory Logic
    PUBLISH_DIR = os.getenv("PUBLISH_DIR", "docs")
    os.makedirs(PUBLISH_DIR or ".", exist_ok=True)
    out_path = os.path.join(PUBLISH_DIR, "plan.json") if PUBLISH_DIR else "plan.json"
